...
── PROVENANCE ────────────────────────────────────────────────────────────────
Voll entkoppelte Kopie von `fabrik/audio/pipeline.py` (produktionserprobt).
Bewusst NICHT neu geschrieben: der Blueprint stuft die TTS/ffmpeg-Schicht als
§9-„incidental" ein und nennt den Audio-Merge (`part_offsets`) „the existing proof
that cast-once works" (§2/§13.6). Diese Datei trägt die §Stage-C-Invarianten bereits
(Point-of-no-Return, Chunk-Normalisierung, LUFS-Master). Braucht den venv-Pfad
(pydub/numpy/pyloudnorm + ffmpeg) — daher NICHT von den stdlib-Unit-Tests importiert;
der dünne Adapter (jobs.py/voicing.py/voice_manifest.py) wird getestet, dieser Kern nicht.
Einzige Änderung ggü. Original: entkoppelter textproc-Import (unten).
──────────────────────────────────────────────────────────────────────────────
"""
...
def merge_parts_to_episode(part_paths, episode_path, pause_between_parts_ms, target_lufs,
    ...
    # temp + os.replace: eine beim Kill truncierte MP3 darf nie als fertige
    # Episode liegen bleiben (Fix aus fabrik/audio/pipeline.py nachgezogen,
    # Bugfix-Session 19.07.2026).
    ...
    # ließe sonst weder WAVs noch Offsets zurück, die Post-Merge-Schritte
    # wären nie mehr nachholbar.
